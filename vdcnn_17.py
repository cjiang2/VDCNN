...
conv_initializer = tf.contrib.keras.initializers.he_uniform()
linear_initializer = tf.contrib.keras.initializers.he_uniform()

def Convolutional_Block(inputs, num_filters, name, is_training):
	...
	with tf.variable_scope("conv_block_%s" % name):
		...
		w = tf.get_variable(name='W_2', shape=filter_shape, 
			initializer=conv_initializer)
		b = tf.get_variable(name='b_2', shape=[num_filters], 
				initializer=tf.constant_initializer(0.0))
...
class VDCNN():
	def __init__(self, num_classes, l2_reg_lambda=0.0005, sequence_max_length=1014, num_quantized_chars=69, embedding_size=16):
		...
		self.conv_block_1 = Convolutional_Block(self.first_conv, num_filters=64, name='1', is_training=self.is_training)
		self.conv_block_2 = Convolutional_Block(self.conv_block_1, num_filters=64, name='2', is_training=self.is_training)
		self.pool1 = tf.nn.max_pool(self.conv_block_2, ksize=[1, 3, 1, 1], strides=[1, 2, 1, 1], padding='SAME', name="pool_1")

		self.conv_block_3 = Convolutional_Block(self.pool1, num_filters=128, name='3', is_training=self.is_training)
		self.conv_block_4 = Convolutional_Block(self.conv_block_3, num_filters=128, name='4', is_training=self.is_training)
		self.pool2 = tf.nn.max_pool(self.conv_block_4, ksize=[1, 3, 1, 1], strides=[1, 2, 1, 1], padding='SAME', name="pool_2")

		self.conv_block_5 = Convolutional_Block(self.pool2, num_filters=256, name='5', is_training=self.is_training)
		self.conv_block_6 = Convolutional_Block(self.conv_block_5, num_filters=256, name='6', is_training=self.is_training)
		self.pool3 = tf.nn.max_pool(self.conv_block_6, ksize=[1, 3, 1, 1], strides=[1, 2, 1, 1], padding='SAME', name="pool_3")

		self.conv_block_7 = Convolutional_Block(self.pool3, num_filters=512, name='7', is_training=self.is_training)
		self.conv_block_8 = Convolutional_Block(self.conv_block_7, num_filters=512, name='8', is_training=self.is_training)

		transposed = tf.transpose(self.conv_block_8, [0,3,2,1])
